...
describe("referenceMetadataUtils", () => {
  it("normalizes common metadata shapes", () => {
    expect(normalizeReferenceMetadata({ author_name: "The Channel", duration: 274, provider_name: "YouTube", thumbnail_url: "https://example.com/thumb.jpg", title: "Song Title" })).toMatchObject({
  ...
  it("extracts timestamp markers from available metadata text", () => {
    const metadata = normalizeReferenceMetadata({ description: "0:00 Intro\n0:12 Verse\n0:48 Chorus", title: "Song Title" });
    expect(metadata.extractedMarkers).toHaveLength(3);
    expect(metadata.extractedMarkers[1]).toMatchObject({ label: "Verse", seconds: 12 });
  });
  ...
  it("builds oEmbed URLs", () => {
    const url = buildOEmbedUrl({ platform: "youtube", url: "https://www.youtube.com/watch?v=abc123" });
  ...
  it("adds fallback source labels to oEmbed metadata", () => {
    expect(normalizeOEmbedMetadata({ title: "Video Title" }, { platform: "youtube", platformLabel: "YouTube" })).toMatchObject({
      providerName: "YouTube",
      source: "youtube",
      sourceLabel: "YouTube",
      title: "Video Title",
